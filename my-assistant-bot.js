...
const IMAGE_EDITING_SECTION = `**🖼️ Как оформить это фото как маркетинговый креатив (инструкции для Canva/Photoshop):**
Дай конкретные инструкции дизайнера. Для каждого применимого пункта опиши точное действие, текст, позицию и стиль:
- Логотип/водяной знак: позиция (например: "нижний правый угол"), размер (например: "10-15% ширины изображения"), цвет (белый / тёмный), прозрачность (например: "70% opacity")
- Текст-оверлей на изображении: точный текст (например: "Done in 10 Days"), позиция (например: "нижняя треть по центру"), стиль (например: "белый жирный sans-serif, 48pt, на полупрозрачной тёмной подложке #000 50%")
- Кадрирование: нужно ли и как именно обрезать для лучшей композиции (например: "обрезать 15% снизу и 10% слева -- убрать незаконченный край стены, сместить фокус на плитку")
- Раскладка до/после: нужна ли, как разделить (например: "разделить по вертикали 50/50, добавить плашки 'BEFORE' / 'AFTER' сверху в каждой половине")
- Графические элементы: что добавить и куда (например: "бейдж 'Free Estimate' -- верхний левый угол, белый текст на тёмно-синем круге"; "стрелка вниз у CTA-кнопки"; "тег локации 'Buffalo Grove, IL' -- нижний левый угол, иконка pin + текст")
- Цветокоррекция: конкретная правка тона для соответствия бренду (например: "добавить тёплый оттенок +15, поднять контраст +10, убрать холодную синеву -- фото сейчас выглядит стерильно")
- Контактная информация: нужна ли на самом изображении, что именно и где (например: "номер телефона -- нижний левый угол, белый, 18pt, под логотипом")
- Маркетинговый приём: какую технику применить (например: "добавить оверлей с отзывом клиента -- цитата 3-5 слов крупным шрифтом поверх фото"; "статистика 'Over 50 Bathrooms Remodeled' -- верхняя треть"; "элемент срочности 'Book April -- 2 spots left' -- яркая плашка сверху")`;
